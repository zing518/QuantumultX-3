...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      $.UNs = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
      $.index = i + 1;
      $.isLogin = true;
      $.nN = '';
      message = '';
      await TotalBean();
      console.log(`\n******开始【京东账号${$.index}】${$.nN || $.unmmm}*********\n`);
      if (!$.isLogin) {
        $.msg($.name, `【提示】cookie已失效`, `京东账号${$.index} ${$.nN || $.unmmm}\n请重新登录获取\nhttps://bean.m.jd.com/`, {"open-url": "https://bean.m.jd.com/"});

        if ($.isNode()) {
          await notify.sendNotify(`${$.name}cookie已失效 - ${$.unmmm}`, `京东账号${$.index} ${$.unmmm}\n请重新登录获取cookie`);
        }
        continue
      }
      ...
      try {
        if (err) {
          ...
          if (data) {
            ...
            if (data['retcode'] === 0) {
              $.nN = (data['base'] && data['base'].nN) || $.unmmm;
            } else {
              $.nN = $.unmmm
            }
          } else {
